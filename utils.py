...
path = "./SDI1/SequenceDigitInterference"

path_misc = "./SDI1_miscs/"
...
def finger_melt_IPIs(subj: pd.DataFrame) -> pd.DataFrame:
    ...
    subj_melted = pd.melt(subj, 
                    id_vars=['BN', 'TN', 'SubNum', 'group', 'hand', 'isTrain', 'seq', 'windowSize', 'digitChangePos', 'digitChangeValue', 'isError', 'timingError', 'isCross', 'crossTime'], 
...
def finger_melt_presses(subj: pd.DataFrame) -> pd.DataFrame:
    ...
    subj_melted = pd.melt(subj, 
                    id_vars=['BN', 'TN', 'SubNum', 'group', 'hand', 'isTrain', 'seq', 'windowSize', 'digitChangePos', 'digitChangeValue', 'isError', 'timingError'], 
...
def finger_melt(subj: pd.DataFrame) -> pd.DataFrame:
    ...
    merged_df = melt_IPIs.merge(melt_presses, on = ['BN', 'TN', 'SubNum', 'group', 'hand', 'isTrain', 'seq', 'windowSize',
                                               'digitChangePos', 'digitChangeValue', 'isError', 'timingError', 'N'])\
                                               .merge(melt_responses, on = ['BN', 'TN', 'SubNum', 'group', 'hand', 'isTrain', 'seq', 'windowSize',
                                               'digitChangePos', 'digitChangeValue', 'isError', 'timingError', 'N'] )
...
def is_digit_changed(row: pd.Series):
    ...
    return (int(row['seq'][row['digitChangePos']]) != row['digitChangeValue']) & row['is_trained_seq']
